...
export const Txn = ({ from, to, txn }: any): any => {
  ...
  return (
    <div className="flex rounded-md bg-slate-800 my-2 flex-col w-full p-4">
      ...
      <div className="text-4xl flex flex-row font-black">
        <div className="mr-1 text-slate-400">$</div>
        <span className="text-white">{txn.amount}</span>
      </div>
      <div className="flex flex-row">
        {txn.ipfs && (
          <img
            className="w-32 h-auto mt-3 mr-3 rounded-md outline outline-3 outline-slate-500"
            src={txn.ipfs}
            alt=""
          />
        )}
        <div className="mt-3 w-full rounded-md bg-slate-700 p-3 relative outline outline-3 outline-slate-900">
          <span className="text-slate-500 absolute -top-7 left-0 font-bold">
            note:{' '}
          </span>
          <b>{txn.note}</b>
        </div>
      ...
      <a
        target="_blank"
        rel="noreferrer"
        href={`https://goerli.etherscan.io/tx/${txn.hash}`}
      >
        <div className="p-2 hover:outline outline-4 outline-black font-bold p-2 px-4 bg-yellow-500 rounded-md w-fit mt-4">
          <span className="text-yellow-700">tnx #</span>
          <span className="text-black">
            {getFirstAndLast5LettersFromString(txn.hash)}
          </span>
        </div>
      </a>
      <div className="flex flex-row">
        ...
        >
          <div className="p-2 hover:outline outline-4 outline-black font-bold p-2 px-4 bg-red-500 rounded-md w-fit mt-4">
            <span className="text-red-700">from @</span>
            <span className="text-black">
              {getFirstAndLast5LettersFromString(from)}
            </span>
          </div>
        </a>
        ...
        >
          <div className="p-2 hover:outline outline-4 outline-black font-bold p-2 px-4 bg-green-500 rounded-md w-fit mt-4">
            <span className="text-green-700">to @</span>
            <span className="text-black">
              {getFirstAndLast5LettersFromString(to)}
            </span>
          </div>
        </a>
      </div>
